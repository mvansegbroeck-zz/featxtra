// featbin/compute-gabor-feats.cc

// Copyright 2014  SAIL (authors: JG & DB)
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Create Gabor feature files.\n"
        "Usage:  compute-gabor-feats [options...] <wav-rspecifier> <feats-wspecifier>\n";

    // construct all the global objects
    ParseOptions po(usage);
    GaborOptions gabor_opts;
    ...
    // Register the Gabor option struct
    gabor_opts.Register(&po);
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      if (gabor_opts.frame_opts.samp_freq != wave_data.SampFreq())
        KALDI_ERR << "Sample frequency mismatch: you specified "
                  << gabor_opts.frame_opts.samp_freq << " but data has "
